...
export function legBetween(from, to) {
  ...
  // Normal: walk short hops, ride medium, transit when far. No-walk: replace the
  // walks with a rideshare (true door-to-door), keeping transit only for long hauls.
  const pick = walkMin <= 9 ? "walk" : km <= 9 ? "ride" : "transit";
  const pickNoWalk = km <= 9 ? "ride" : "transit";
