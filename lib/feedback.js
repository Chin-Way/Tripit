...
// ("use my review to improve Triperly") into a structured signal that:
...
export function groundingText(candidates = []) {
  const lines = candidates
    ...
    .slice(0, 8)
    .map((v) => `- ${v.name}: ${v.familySignal.avg.toFixed(1)}/5 from ${v.familySignal.n} Triperly famil${v.familySignal.n === 1 ? "y" : "ies"}`);
  if (!lines.length) return "";
  return `\n\nTRIPERLY FAMILY SIGNALS (aggregated from families who consented to share their reviews — gently prefer the better-reviewed venues when choosing):\n${lines.join("\n")}`;
}
