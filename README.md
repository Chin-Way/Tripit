...
Family trips, planned around the nap. Voyet turns a 60-second family survey into a
...
Voyet supports two AI providers — **Claude** (default) or **Gemini** — chosen with
...
Set `GOOGLE_PLACES_API_KEY` and Voyet pulls **real venues** (ratings, locations,
...
> tables) are still partly derived in `lib/places.js` — that curation is Voyet's
...
- **`lib/places.js`** — live venue data from the **Google Places API** (used when
  `GOOGLE_PLACES_API_KEY` is set), mapped into Voyet's schema and cached per city.
- **`data/chicago.json`** — the curated *seed* venue list (real ratings + family
  ...
  truth for grounding — Voyet never invents hours or places.
...
Voyet now has an optional backend for accounts, saved trips, family reviews, and
...
Reviews where a family ticked **"use my review to improve Voyet"** become a structured
...
- **Consent:** the "use my review to improve Voyet" checkbox is **opt-in (off by
  default)** and stored per review. Reviews can be deleted.
- **Children's data:** accounts are for parents/adults. Voyet stores only kids' **age
...
- (Optional) Paste your `ANTHROPIC_API_KEY` in the dashboard to turn on the AI
  engine. Skip it and Voyet still works on the rules engine.
- Render gives you a URL like `https://voyet.onrender.com`.
...
npm run qr -- https://voyet.onrender.com
```
...
**Why one QR does both jobs:** Voyet is a PWA, so scanning it on a phone lets
